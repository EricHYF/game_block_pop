...
namespace GKIT
{
    ...
    public class UIBinderInspectorExtends: UnityEditor.Editor
    {
        ...
        {
            Application.OpenURL($"file:///{Path.GetFullPath(_binder.bindScriptPath)}");
        }
    ...
    class BinderDataInjectior
    {
        ...
        private const string K_USING_GKIT_UI = "GKIT.UI";
        private const string K_USING_GKIT = "GKIT";
        ...
        private static string C_ONCREATE_LINE = "\t\tprotected override void OnCreateOver()";
        private static string C_CALL_BASE_ONCREATE= "\t\t\tbase.OnCreateOver(); // 一定要调用基类的方法, 会有一些基础组件的初始化";
        ...
        public BinderDataInjectior SetVariables(VariableArray variables, string scope)
        {
            ...
            states.Insert(0, C_CALL_BASE_ONCREATE);
            ...
            if (onCreateCall < 0)
            {
                states.Insert(0, C_ONCREATE_LINE);
                states.Insert(1, "\t\t{");
                states.Add("\t\t}");
